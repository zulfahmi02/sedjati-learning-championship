    ...
    return (
        <div className="group relative flex flex-col gap-3 overflow-hidden rounded-3xl border-2 border-leaf/10 bg-white p-5 shadow-[0_4px_0_rgba(42,51,31,0.06)] transition-transform duration-200 hover:-translate-y-0.5 dark:bg-card">
            ...
            <div className="flex items-center justify-between">
                <span className="font-heading text-xs font-bold tracking-wider text-ink/60 uppercase dark:text-muted-foreground">
                    {label}
                </span>
                {Icon && (
            ...
            <div className="relative font-heading text-4xl font-bold text-deep dark:text-card-foreground">
                {value}
            </div>
            {detail && (
                <div className="border-t border-leaf/10 pt-2 text-xs font-semibold text-ink/60 dark:text-muted-foreground">
                    {detail}
                </div>
            )}
        </div>
