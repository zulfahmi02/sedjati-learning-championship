...
export default function PublicLayout({
    ...
    return (
        <div className="relative flex min-h-screen flex-col overflow-x-hidden bg-gradient-to-b from-butter to-[#FFFDF6]">
            ...
            <header className="fixed top-0 right-0 left-0 z-40 border-b border-leaf/10 bg-white/80 backdrop-blur-md">
                <div className="mx-auto flex h-16 w-full max-w-[1440px] items-center justify-between gap-4 px-4 md:px-6">
                    <div className="flex items-center gap-4">
                        <Link
                            href={home()}
                            className="font-heading text-lg font-bold text-deep"
                        >
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        {auth.user ? (
                            <Link
                                href={dashboard()}
                                className="inline-flex items-center gap-2 rounded-2xl border-2 border-leaf bg-leaf px-5 py-2 font-heading text-sm font-bold text-white shadow-sm transition-transform active:translate-y-0.5"
                            >
